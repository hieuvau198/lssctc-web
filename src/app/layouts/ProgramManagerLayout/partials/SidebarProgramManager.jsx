...
import { LayoutDashboard, FlaskConical, Calendar, PanelLeft, PanelLeftClose, MoreVertical } from "lucide-react";
    ...
    <>
      ...
      >
        ...
        <div className={`flex items-center h-16 border-b border-slate-200/60 ${collapsed ? 'justify-center px-2' : 'justify-between px-4'}`}>
          {collapsed ? (
            <Tooltip placement="right" title="Program Manager">
              <button
                onClick={onToggle}
                className="flex items-center justify-center bg-gradient-to-br from-emerald-500 to-teal-600 text-white font-bold rounded-xl size-10 text-lg shadow-lg shadow-emerald-200/50 hover:scale-105 transition-transform"
              >
                P
              </button>
            </Tooltip>
            ...
            <>
              <div className="flex items-center gap-3 overflow-hidden">
                <div className="flex items-center justify-center bg-gradient-to-br from-emerald-500 to-teal-600 text-white font-bold rounded-xl size-10 shrink-0 text-lg shadow-lg shadow-emerald-200/50">
                  P
                </div>
                <span className="font-bold text-transparent bg-clip-text bg-gradient-to-r from-emerald-600 to-teal-600 tracking-wide">
                  Program Manager
                </span>
              </div>
              ...
              <button
                onClick={onToggle}
                className="hidden md:inline-flex w-9 h-9 items-center justify-center rounded-xl border border-slate-200 hover:bg-emerald-50 hover:border-emerald-300 transition-all duration-200"
              ...
              <button
                ...
                className="md:hidden inline-flex w-9 h-9 items-center justify-center rounded-xl border border-slate-200 hover:bg-emerald-50 hover:border-emerald-300 transition-all duration-200"
                ...
                >
                  {collapsed ? (
                    <Tooltip placement="right" title={item.label}>
                      <span className="text-lg flex-shrink-0 mx-auto">
                        {item.icon}
                      </span>
                    </Tooltip>
                  ) : (
                ...
                <Menu
                  onClick={async ({ key }) => {
                    if (key === 'logout') {
                      ...
                      navigate('/programManager/profile');
                    }
                  }}
                  items={[
                    {
                  ...
                  <Dropdown overlay={menu} placement="topCenter" trigger={["click"]}>
                    <div className="cursor-pointer">
                      <Tooltip placement="right" title={<div className="text-sm text-left"><div className="font-semibold">{fullName}</div><div className="text-xs text-gray-400">{role}</div></div>}>
                        <div className="flex items-center justify-center p-2 rounded-xl hover:bg-emerald-50 transition-colors">
                          {(() => {
                            ...
                            return resolvedAvatar ? (
                              <Avatar size={32} src={resolvedAvatar} className="ring-2 ring-emerald-200" />
                            ) : (
                              <Avatar size={32} className="bg-gradient-to-br from-emerald-500 to-teal-600 text-white font-semibold">{avatarContent}</Avatar>
                            );
                          })()}
                        </div>
                      </Tooltip>
                  ...
                  <div className="bg-white/80 backdrop-blur-sm shadow-lg shadow-slate-200/50 ring-1 ring-slate-200/60 rounded-2xl w-full flex items-center justify-between px-3 py-2.5">
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                      {(() => {
                        ...
                        if (avatarUrl && avatarUrl !== persisted) setAvatarUrl(avatarUrl);
                        return resolvedAvatar ? (
                          <Avatar size={40} src={resolvedAvatar} className="ring-2 ring-emerald-200" />
                        ) : (
                          <Avatar size={40} className="bg-gradient-to-br from-emerald-500 to-teal-600 text-white font-semibold">{avatarContent}</Avatar>
                        );
                      })()}
                    ...
                    <Dropdown overlay={menu} placement="topRight" trigger={["click"]} arrow={{ pointAtCenter: true }}>
                      <button className="p-2 rounded-xl hover:bg-emerald-50 transition-colors" aria-label="user menu">
