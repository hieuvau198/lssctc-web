...
import { LayoutDashboard, Users, Layers, BookOpen, Calendar, PanelLeftClose, PanelLeft, MoreVertical } from 'lucide-react';
          ...
          {collapsed ? (
            ...
            <>
              <div className="flex items-center gap-3 overflow-hidden">
                <div className="flex items-center justify-center bg-gradient-to-br from-cyan-500 to-blue-600 text-white font-bold rounded-xl size-10 shrink-0 text-lg shadow-lg shadow-cyan-200/50">
                  A
                </div>
                <span className="font-bold text-transparent bg-clip-text bg-gradient-to-r from-cyan-600 to-blue-600 tracking-wide">
              ...
              <button
                onClick={onToggle}
                className="hidden md:inline-flex w-9 h-9 items-center justify-center rounded-xl border border-slate-200 hover:bg-cyan-50 hover:border-cyan-300 transition-all duration-200"
              ...
              <button
                ...
                className="md:hidden inline-flex w-9 h-9 items-center justify-center rounded-xl border border-slate-200 hover:bg-cyan-50 hover:border-cyan-300 transition-all duration-200"
                  ...
                  <Dropdown overlay={menu} placement="topCenter" trigger={["click"]}>
                    <div className="cursor-pointer">
                      <Tooltip placement="right" title={<div className="text-sm text-left"><div className="font-semibold">{fullName}</div><div className="text-xs text-gray-400">{role}</div></div>}>
                        <div className="flex items-center justify-center p-2 rounded-xl hover:bg-cyan-50 transition-colors">
                          {(() => {
                            ...
                            return resolvedAvatar ? (
                              <Avatar size={32} src={resolvedAvatar} className="ring-2 ring-cyan-200" />
                            ) : (
                              <Avatar size={32} className="bg-gradient-to-br from-cyan-500 to-blue-600 text-white font-semibold">{avatarContent}</Avatar>
                            );
                          })()}
                        </div>
                      </Tooltip>
                  ...
                  <div className="bg-white/80 backdrop-blur-sm shadow-lg shadow-slate-200/50 ring-1 ring-slate-200/60 rounded-2xl w-full flex items-center justify-between px-3 py-2.5">
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                      {(() => {
                        ...
                        if (avatarUrl && avatarUrl !== persisted) setAvatarUrl(avatarUrl);
                        return resolvedAvatar ? (
                          <Avatar size={40} src={resolvedAvatar} className="ring-2 ring-cyan-200" />
                        ) : (
                          <Avatar size={40} className="bg-gradient-to-br from-cyan-500 to-blue-600 text-white font-semibold">{avatarContent}</Avatar>
                        );
                      })()}
                    ...
                    <Dropdown overlay={menu} placement="topRight" trigger={["click"]} arrow={{ pointAtCenter: true }}>
                      <button className="p-2 rounded-xl hover:bg-cyan-50 transition-colors" aria-label="user menu">
