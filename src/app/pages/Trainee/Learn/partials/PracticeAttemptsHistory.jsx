        ...
        <div className="bg-white border-2 border-black mt-6">
            <div className="px-4 py-3 bg-neutral-100 border-b-2 border-black flex items-center justify-between">
                <h3 className="font-black text-black uppercase flex items-center gap-2">
                    ...
                    {t('trainee.practice.history', 'Lịch sử làm bài')} ({attempts.length})
                </h3>
            </div>
                    ...
                    <div key={attempt.id} className="bg-white">
                        ...
                        >
                            <div className="flex items-center gap-4">
                                <div className={`w-10 h-10 border-2 border-black flex items-center justify-center flex-shrink-0 ${attempt.isPass ? 'bg-green-200' : 'bg-red-400'}`}>
                                ...
                                <div>
                                    ...
                                    <div className="flex items-center gap-4 text-sm text-neutral-600 mt-1">
                                        <span className="flex items-center gap-1">
                                            ...
                                            Điểm: <span className="font-bold text-black">{attempt.score ?? 0}</span>
                                        </span>
                                        <span className="flex items-center gap-1">
                                            <AlertCircle className="w-3 h-3" /> 
                                            Lỗi: <span className="font-bold text-black">{attempt.totalMistakes ?? 0}</span>
                                        </span>
                                    </div>
                                </div>
                            </div>
                        ...
                        {expandedId === attempt.id && (
                            <div className="border-t-2 border-neutral-200 bg-neutral-50 p-4">
                                <h4 className="text-xs font-bold uppercase text-neutral-500 mb-3 ml-1">Nhiệm vụ thành phần</h4>
                                        ...
                                        <div key={task.id} className="flex items-center justify-between p-3 bg-white border border-neutral-300 shadow-sm">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <span className={`${task.isPass ? 'text-black' : 'text-neutral-500'}`}>
                                                    {task.taskName || `Task #${task.taskId}`}
                                                </span>
                                            </div>
                                            <div className="text-sm font-mono">
                                                <span className="text-neutral-500">Lỗi:</span> <b>{task.mistakes ?? 0}</b>
                                                <span className="mx-2 text-neutral-300">|</span>
                                                <span className="text-neutral-500">Điểm:</span> <b>{task.score}</b>
                                            </div>
                                        </div>
                                    ))}
                                </div>
