                    ...
                    <div key={attempt.id} className="bg-white group">
                        ...
                        >
                            <div className="flex items-start md:items-center gap-4">
                                {/* Rank/Order Badge */}
                                <div className={`w-12 h-12 border-2 border-black flex items-center justify-center flex-shrink-0 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] ${attempt.isPass ? 'bg-green-400' : 'bg-red-400'}`}>
                                ...
                                <div>
                                    ...
                                    <div className="flex flex-wrap items-center gap-4 text-sm text-neutral-600 mt-2">
                                        <span className="flex items-center gap-1.5 font-bold border-2 border-neutral-200 px-2 py-0.5 bg-neutral-50">
                                            ...
                                            Score: <span className="text-black">{attempt.attemptScore} / {attempt.maxScore}</span>
                                        </span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="flex items-center justify-between md:justify-end gap-4 w-full md:w-auto pl-[4rem] md:pl-0">
                                <div className={`px-4 py-1.5 border-2 border-black font-bold uppercase text-sm shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] flex items-center gap-2 ${attempt.isPass ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                                    {attempt.isPass ? (
                                            ...
                                            <div className="flex items-start gap-3">
                                                ...
                                                <div className="flex-grow">
                                                    <div className="flex justify-between items-start gap-4">
                                                        ...
                                                        <span className={`text-xs font-mono font-bold px-2 py-1 border border-neutral-200 ${question.isCorrect ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'}`}>
                                                        ...
                                                        <div className="mt-3 pl-2 border-l-2 border-neutral-200 space-y-1">
                                                            {question.quizAttemptAnswers.map((ans) => (
                                                                <div key={ans.id} className="text-xs flex items-center gap-2 text-neutral-600">
                                                                    <div className={`w-1.5 h-1.5 rounded-full ${ans.isCorrect ? 'bg-green-500' : 'bg-red-500'}`} />
                                                                    <span>{ans.name}</span>
                                                                </div>
                                                            ))}
                                                        </div>
