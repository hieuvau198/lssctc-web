...
export default function ArticlesSection() {
  ...
  <div className="max-w-[1380px] mx-auto px-5 sm:px-6 md:px-10">
        <div className="flex items-baseline justify-between mb-8">
          <h2 className="text-xl md:text-2xl font-semibold text-slate-900 tracking-tight">Insights & Articles</h2>
          <a href="#" className="text-blue-600 text-sm hover:text-blue-700 font-medium">More posts</a>
        </div>
        <div className="grid md:grid-cols-3 gap-7">
          {articles.map(a => (
            <article key={a.id} className="group flex flex-col rounded-xl border border-slate-200 bg-white p-5 shadow-sm hover:border-blue-500 hover:shadow-md transition-colors">
              ...
              <p className="mt-3 text-[11px] text-slate-600 line-clamp-3">Short teaser placeholder. Replace with summary excerpt from CMS.</p>
              <button className="mt-4 text-[11px] font-semibold text-blue-600 hover:text-blue-700 inline-flex items-center gap-1">Read <span aria-hidden>→</span></button>
            </article>
          ))}
        </div>
      </div>
