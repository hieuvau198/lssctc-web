...
const MaterialManagement = () => {
  ...
  const handleDelete = (id) => {
    modal.confirm({
      title: t('common.deleteConfirm', 'Are you sure you want to delete this material?'),
  ...
  const handleFormSubmit = async (values) => {
    ...
    try {
      ...
      await apiCall();
      message.success(isEdit ? t('common.updateSuccess', 'Material updated') : t('common.createSuccess', 'Material created'));
      setDrawerOpen(false);
      fetchData();
    } catch (error) {
      console.error(error);
      message.error(error.message || (editingMaterial ? t('common.updateError', 'Update failed') : t('common.createError', 'Create failed')));
    } finally {
      setSubmitting(false);
    }
  };
      ...
      render: (_, record) => (
        record.url ? (
            ...
            >
                ...
                {record.learningMaterialType === 'File' ? 'Download' : 'Visit'}
            </a>
        ) : <span className="text-neutral-400 italic">No Link</span>
      )
    },
    {
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h3 className="text-lg font-black uppercase tracking-wider text-white m-0">
                                {editingMaterial ? t('common.editMaterial', 'Edit Material') : t('common.addMaterial', 'New Material')}
                            </h3>
                        </div>
                    </div>
                    <button
