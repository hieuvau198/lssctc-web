...
const MaterialForm = ({ initialValues, onFinish, loading, onCancel }) => {
  ...
  return (
    ...
    >
      <Form.Item
        name="name"
        label={<span className="font-bold uppercase text-xs">{t('material.materialName')}</span>}
      ...
      <Form.Item
        name="description"
        label={<span className="font-bold uppercase text-xs">{t('common.description')}</span>}
      >
        <TextArea
          rows={3}
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <Form.Item
          ...
          label={<span className="font-bold uppercase text-xs">{t('material.contentType')}</span>}
          rules={[{ required: true }]}
        >
          <Select className="border-neutral-400">
            <Option value="Document">
              <div className="flex items-center gap-2">
                <FileText size={14} />
                <span>{t('material.document')}</span>
              </div>
            </Option>
            <Option value="Video">
              <div className="flex items-center gap-2">
                <Video size={14} />
                <span>{t('material.video')}</span>
              </div>
            </Option>
          </Select>
        </Form.Item>
        ...
        <Form.Item
          name="sourceType"
          label={<span className="font-bold uppercase text-xs">{t('material.source')}</span>}
          rules={[{ required: true }]}
        >
          <Select
          ...
          >
            <Option value="Link">
              <div className="flex items-center gap-2">
                <LinkIcon size={14} />
                <span>{t('material.externalLink')}</span>
              </div>
            </Option>
            <Option value="File">
              <div className="flex items-center gap-2">
                <UploadIcon size={14} />
                <span>{t('material.fileUpload')}</span>
              </div>
            </Option>
          </Select>
        </Form.Item>
      ...
      {sourceType === 'Link' ? (
        <Form.Item
          name="url"
          label={<span className="font-bold uppercase text-xs">{t('material.urlAddress')}</span>}
          rules={[
            { required: true, message: t('common.required', 'This field is required') },
            { type: 'url', message: t('material.invalidUrl') }
          ]}
        >
          <Input
        ...
        <Form.Item
          name="upload"
          label={<span className="font-bold uppercase text-xs">{t('material.file')}</span>}
        ...
        >
          ...
          >
            <div className="flex flex-col items-center justify-center text-neutral-500">
              ...
              <div className="text-xs uppercase font-bold">{t('material.uploadPlaceholder')}</div>
            </div>
          </Upload>
        </Form.Item>
      )}
      ...
      {initialValues && sourceType === 'File' && initialValues.url && (
        <div className="mb-6 p-3 bg-neutral-100 border border-neutral-300 rounded text-sm">
          <span className="font-bold text-xs uppercase block text-neutral-500 mb-1">{t('material.currentFile')}:</span>
          <a href={initialValues.url} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline break-all">
            {initialValues.url ? initialValues.url.split('?')[0].split('/').pop() : t('material.viewFile')}
          </a>
        </div>
      )}
