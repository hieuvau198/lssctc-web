...
const QuizManagement = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      message.error(t('common.fetchError', 'Failed to load quizzes'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = (id) => {
    modal.confirm({
      title: t('common.deleteConfirm', 'Are you sure you want to delete this quiz?'),
      content: t('common.deleteWarning', 'This action cannot be undone.'),
      ...
      onOk: async () => {
        try {
          await deleteQuiz(id);
          message.success(t('common.deleteSuccess', 'Quiz deleted successfully'));
          fetchData();
        } catch (error) {
          message.error(error.message || t('common.deleteError', 'Failed to delete quiz'));
        }
      },
    });
  };
  ...
  const handleEdit = async (record) => {
    try {
      ...
      message.error(t('instructor.quizzes.messages.loadQuizFailed', 'Failed to load quiz details'));
    } finally {
        setLoading(false);
    }
  };
  ...
  const columns = [
    ...
    {
        title: t('instructor.quizzes.table.timeMinutes', 'Time Limit'),
        dataIndex: 'timelimitMinute',
        key: 'timelimitMinute',
        width: 120,
        align: 'center',
        render: (v) => <span className="font-bold">{v} mins</span>
    },
    {
        title: t('instructor.quizzes.table.passScore', 'Pass Score'),
    ...
    {
      title: t('instructor.quizzes.table.totalScore', 'Total Score'),
        ...
        <div className="bg-neutral-800 border-2 border-black px-4 py-3 mb-0 flex-none z-10">
            ...
            <div className="flex items-center gap-3">
                ...
                <p className="text-yellow-400 text-xs mt-0.5 font-bold">
                    {filteredQuizzes.length} {t('common.items', 'items')}
                </p>
                </div>
            </div>
