...
export default function ClassTimeslotSchedule({ classId, className }) {
    ...
    const getStatusStyle = (status) => {
        const statusMap = {
            ...
            'Ongoing': { bg: 'bg-yellow-400', text: 'text-black', border: 'border-black', label: isVietnamese ? 'Đang diễn ra' : 'Ongoing' },
            'Completed': { bg: 'bg-black', text: 'text-yellow-400', border: 'border-black', label: isVietnamese ? 'Hoàn thành' : 'Completed' },
    ...
    return (
        <div className="bg-white border-2 border-black overflow-hidden">
            ...
            <div className="p-4 border-b-2 border-neutral-100 flex flex-wrap gap-3 items-center bg-neutral-50">
                <span className="text-sm text-neutral-600 font-bold uppercase">{isVietnamese ? 'Chú thích' : 'Legend'}:</span>
                <span className="px-2 py-1 text-xs font-bold uppercase bg-neutral-100 text-neutral-600 border border-neutral-300">{isVietnamese ? 'Chưa bắt đầu' : 'Not Started'}</span>
                <span className="px-2 py-1 text-xs font-bold uppercase bg-yellow-400 text-black border border-black">{isVietnamese ? 'Đang diễn ra' : 'Ongoing'}</span>
                <span className="px-2 py-1 text-xs font-bold uppercase bg-black text-yellow-400 border border-black">{isVietnamese ? 'Hoàn thành' : 'Completed'}</span>
                <span className="px-2 py-1 text-xs font-bold uppercase bg-red-100 text-red-700 border border-red-400">{isVietnamese ? 'Đã hủy' : 'Cancelled'}</span>
            </div>
            ...
            <Table
                ...
                locale={{
                    emptyText: (
                        <Empty
                            description={<span className="font-bold uppercase text-neutral-500">{isVietnamese ? 'Chưa có lịch học nào' : 'No schedule available'}</span>}
                        />
                    ),
                }}
            />
