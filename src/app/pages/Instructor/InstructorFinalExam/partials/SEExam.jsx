...
export default function SEExam({ classId }) {
  ...
  // Filter only SE (Simulation Exam) type exams
  const seExams = exams.filter(exam => exam.type === 'SE' || exam.name?.includes('Simulation'));
      ...
      <div className="mb-4 flex justify-between items-center">
        <div>
          <span className="text-lg font-semibold text-slate-800">
            {t('instructor.finalExam.seTitle')}
          </span>
          <p className="text-sm text-slate-500">{t('instructor.finalExam.seDescription')}</p>
        </div>
