...
export default function PEExam({ classId }) {
  ...
  // Filter only PE (Practical Exam) type exams
  const peExams = exams.filter(exam => exam.type === 'PE' || exam.name?.includes('Practical'));
  ...
  const columns = [
    ...
    {
      title: t('instructor.finalExam.totalTasks'),
    ...
    <div className="py-4">
      <div className="mb-4 flex justify-between items-center">
        <div>
          <span className="text-lg font-semibold text-slate-800">
            {t('instructor.finalExam.peTitle')}
          </span>
          <p className="text-sm text-slate-500">{t('instructor.finalExam.peDescription')}</p>
        </div>
      ...
      >
        <Form form={form} layout="vertical" className="mt-4">
          ...
          <Form.Item
            name="totalTasks"
            label={t('instructor.finalExam.totalTasks')}
            rules={[{ required: true, message: t('instructor.finalExam.totalTasksRequired') }]}
          >
